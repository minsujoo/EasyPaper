...
- fix: 구버전 클라이언트의 Vault 레코드 오분류를 정리하고 전용 프로토콜 권한 없이 파일 변경을 거부하도록 보호
...
- fix: 학회 일정의 미정 탭을 제거하고 월·연도 단위 일정은 추정하며 지난 마감과 종료 학회를 숨김
- feat: 공식 등록 페이지를 별도로 탐색해 저자·조기·일반·현장 등록 일정과 참가 등록 가능 상태 표시
- feat: 학회 일정 카드에 주요 연구 분야를 설명하는 짧은 한국어 안내 추가
- feat: 지난 학회 일정은 숨기고 개최 주기에 따라 다음 회차를 자동 생성하며 과거 일정 기반 추정 날짜 표시
- feat: 86개 학회 공식 사이트 연결과 검증 기반 24시간 일정 갱신 추가
- fix: 공식 페이지의 통지일·튜토리얼 날짜와 행사 이후 날짜를 본 행사·논문 마감으로 오인하지 않도록 보정
- feat: 사용자 제공 학회 시트 163개를 적합도와 마감 상태별 목록으로 통합
- feat: 주석 상단 탭을 제거하고 관계 지도·학회 일정을 독립된 연구 탐색 탭으로 이동
- fix: Semantic Scholar 전용 추천의 지원 필드와 호출 간격·429 재시도 보정
- feat: 장기 평가 기록을 활용하는 개인 취향 모델과 추천 재정렬 추가
- feat: 공개 PDF를 Unpaywall·OpenAlex·Crossref·arXiv에서 순차 탐색하고 저장 상태 병합
- feat: 앱 종료 중에도 작동하는 Ubuntu 사용자 타이머 기반 24시간 논문 수집 추가
- feat: 인용·참고문헌·유사 논문 관계 지도와 주요 AI 학회 플래너 추가
- refactor: Scholar의 다섯 개 피드를 하나의 추천 목록으로 통합하고 점수·수집 상태 UI 정리
- feat: Scholar 북마크를 보관함 폴더 목록으로 이동하고 추천 목록 자동 로드 추가
- feat: Scholar에 최신·놓친 논문·취향 학습·폴더별 북마크 피드와 관련성 점수 추가
- feat: Semantic Scholar 전용 추천 모델, 24시간 정기 수집 캐시와 수동 새 논문 수집 연결
- feat: 검색 결과 중복 노출·숨김·열기 상태 영속화 및 Figure/Table 최대 5개 미리보기 추가
- feat: Obsidian 전체 화면 통합 플러그인과 보관함·Scholar·히스토리·노트·단어장 화면 연결
- feat: Vault PDF 가져오기, 생성 노트·Figure/Table·원문 PDF 양방향 동기화 및 선택 단어 저장
- security: 실행별 로컬 통합 토큰, 사용자 전용 연결 파일 및 Obsidian iframe 세션 추가
- fix: Obsidian 통합 바로가기 해시를 초기 인증이 보관함으로 덮어쓰던 문제 수정
- feat: Anki 스케줄러와 연동되는 단어 카드 복습 모드와 키보드 단축키 추가
- fix: 단어장 카드 목록에 독립 세로 스크롤 영역 추가
- fix: Obsidian 동기화 카드를 앱 단어장에도 표시하고 기존 카드 수를 정확히 안내
- fix: 기존 Obsidian 논문 단어장 카드를 처음 Anki 동기화할 때 자동 마이그레이션
- feat: 논문에서 선택한 영어 단어·구문을 문맥 번역과 함께 저장하는 단어장 추가
- feat: Anki 자동 실행, 로컬 전용 AnkiConnect 연동, 중복 방지 및 오프라인 동기화 대기열 추가
- feat: 기존 Obsidian 플래시카드 덱이 있으면 동일한 카드 형식으로 함께 기록

## 2026-08-01

- fix: 문단별 설명 진입점을 제거하고 PDF 목차 항목을 섹션 설명 기준으로 통일
- fix: 설명 팝업을 최상위 화면 레이어로 이동하고 8방향 크기 조절·닫기 버튼 가시성 개선
- feat: 제목 형식과 무관하게 본문 문단 경계를 인식해 문단별 설명 진입점 제공
- feat: 원문과 번역을 독립 스크롤 열로 분리하고 문장 클릭 시 반대쪽 열만 이동
- fix: 원문·번역 문장 클릭 시 클릭한 쪽은 고정하고 반대쪽 패널만 대응 위치로 스크롤
- fix: 2단 논문의 캡션·각주 및 페이지 경계에서 잘린 본문 문장을 재결합해 번역 흐름과 위치 개선
- feat: 히스토리를 날짜별 논문과 읽기 진도를 표시하는 월간 달력으로 개편
- feat: 논문 마지막 읽기 위치 자동 저장·복원과 뷰어 책갈피 진도 표시 추가
- feat: 독립 노트 탭을 보관함의 논문·노트 보기와 논문 카드별 노트 버튼으로 통합
- feat: Scholar 통합 탭에 맞춤 추천·최신 논문·관심 평가·유사 논문·BibTeX·라이브러리 직접 저장 추가
- fix: AI 검색 결과 영역 스크롤 및 전문용어 문맥 판별·다중 검색·RRF 재정렬·중복 논문 제거 개선
- feat: 홈 화면 AI 논문 검색 추가(OpenAlex 검증, 자연어 검색어 확장, AI 요약·관련성 설명, 연도/무료 원문/정렬 필터)
- feat: 라이브러리 폴더 생성·이름 변경·삭제 및 논문 이동·폴더별 필터 기능 추가
- feat: 데스크톱 파일 관리자에서 PDF를 보관함으로 끌어다 놓는 다중 업로드 지원
- fix: Ubuntu WebKit에서 드롭한 PDF의 asset URL을 읽지 못하던 문제 수정

## 2026-07-31

- feat: 문장·문단, 수식, Figure/Table, 인용, 섹션별 AI 설명 및 문맥 유지 후속 채팅 추가
- feat: PDF 본문의 각 섹션 제목 옆에 인라인 설명 버튼 추가
...
- 논문, 폴더, 번역, 채팅, 노트, 읽기 기록, 단어장과 추천 신호를 여러 기기에서 맞추는 중앙 동기화 서버를 추가했습니다.
- PDF를 SHA-256 기반 파일 저장소에 중복 없이 보관하고, 삭제 기록과 충돌 사본을 남기도록 했습니다.
- 설정에 동기화 서버 주소, 기기 토큰, 자동 주기, 즉시 동기화와 상태 표시를 추가했습니다.
- AI 계정과 API 키, 기기 경로와 캐시는 동기화 대상에서 제외했습니다.
