...
async def stream_page_insight(
    ...
    if kind == "keywords":
        instruction = (
            ...
            f"각 항목은 \"- **원어 용어**: {target_lang} 뜻풀이\" 형식의 마크다운 목록으로만 출력하고, "
            f"서론이나 부연 설명은 절대 추가하지 마세요. 위 기준을 만족하는 용어가 거의 없다면 목록 개수를 "
            f"줄이거나, 정말 하나도 없으면 \"이 페이지에는 특별히 짚을 전문용어/고급 어휘가 없습니다.\"라고만 "
            f"답하세요."
        )
    else:  # "summary"
        instruction = (
