// 프로젝트 전역에서 쓰이는 심플한 라인 아이콘 모음
// Lucide(https://lucide.dev) 공식 아이콘의 path 데이터를 그대로 옮겨와
// 별도 런타임 의존성 없이 인라인 SVG로 렌더링한다.
// 컨벤션: viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
...
export function icon(name, size = 14, attrs = '') {
  const d = PATHS[name]
  if (!d) return ''
  return `<svg width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" ${attrs}>${d}</svg>`
}
